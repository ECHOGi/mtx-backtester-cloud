...
def run_cutoff_scenarios(session_bars: pd.DataFrame, items: list[tuple[str, dict]],
    ...
    for cutoff in cutoffs:
        ...
        for state in SCENARIO_STATES:
            for path_idx in range(config.paths_per_state):
                ...
                bench_future_peak = pd.concat([pd.Series([bench_at_cutoff]), bench_values], ignore_index=True).cummax()
                bench_future_dd = ((pd.concat([pd.Series([bench_at_cutoff]), bench_values], ignore_index=True) /
                                    bench_future_peak) - 1.0).min() * 100
                ...
                for name, cfg in items:
                    ...
                    realized_pnl = float(metrics.get("扣除期末強制平倉後損益(元)", metrics.get("總損益(元)", 0.0)))
                    ...
                    rows.append({
                        ...
                        "總損益含期末浮動(元)": metrics.get("總損益(元)", 0.0),
                        "最大回撤(元)": metrics.get("最大回撤(元)", 0.0),
                        "最大回撤率(%)": metrics.get("策略標準最大回撤率(%)", metrics.get("最大回撤(%)", np.nan)),
                        ...
                        "超越正二": bool(pd.notna(realized_annual) and realized_annual > benchmark_annual),
                        "相對正二年化差(百分點)": round(float(realized_annual - benchmark_annual), 2) if pd.notna(realized_annual) else np.nan,
                    })
                    done += 1
    ...
    for name, grp in dist.groupby("策略名稱", sort=False):
        pnl = pd.to_numeric(grp["已實現損益(元)"], errors="coerce")
        annual = pd.to_numeric(grp["已實現年化報酬率(%)"], errors="coerce")
        dd = pd.to_numeric(grp["最大回撤率(%)"], errors="coerce")
        summaries.append({
            "策略名稱": name,
            "情境路徑數": len(grp),
            "已實現損益中位數": round(float(pnl.median()), 0),
            "已實現損益P25": round(float(pnl.quantile(.25)), 0),
            "已實現損益P10": round(float(pnl.quantile(.10)), 0),
            "已實現年化中位數(%)": round(float(annual.median()), 2),
