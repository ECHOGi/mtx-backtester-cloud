...
在左側「資料與換倉進階」內的「CSV 資料夾路徑」填入該資料夾路徑即可（預設 `data`，
...
左側選擇資料與回測方式；策略設定完成後，按「執行單次回測」或「開始批次回測」才會重新計算。
...
- 可用固定帳戶資金與單筆風險比例，依進場 ATR 停損距離自動計算部位。
- 以微台等值單位配置：1 單位為 1 口微台，5 單位為 1 口小台，最多可設定 10 單位（2 口小台等值）。
- 低波動時增加部位，高波動時縮小為微台，停損點位不因契約大小而改變。
- 可設定跳空壓力倍數；進場時要求「原始保證金＋壓力損失」不超過帳戶可用權益。
- 獲利不會把原始 50 萬風險池越放越大；虧損後則依剩餘權益自動縮小部位。
- 交易明細新增小台／微台口數、每點總價值、風險預算、壓力風險與保證金使用。
- 批次 JSON 可在最上層指定 `initial_capital`，本輪正式測試固定為 500,000 元。

## v0.6.6：ATR 標準化分段出場

- 可用「最大順向浮盈點數 ÷ 進場可用 ATR」設定分段吊燈。
- 進場可用 ATR 固定取訊號根（進場前一根已完成 K 棒），避免使用尚未完成的進場根資料。
- 交易明細新增 `entry_atr` 與 `max_favorable_atr_multiple`。
- 舊版固定金額階梯仍可回測，維持歷史策略相容。


## v0.6.6：ATR 停損與單筆風險上限

- `stop_threshold_mode=entry_atr`：停損距離 = 訊號根 ATR × `stop_atr_multiple`。
- `use_entry_risk_cap=true`：若預定停損距離換算金額超過 `max_entry_risk_amount`，略過該次進場。
- 風險上限只衡量正常停損距離；跳空、滑價與成本仍可能使實際虧損超過上限。
- 交易明細新增預定停損點數、預定停損風險金額與單筆風險上限。
- 批次比較新增風險上限跳過進場次數。
